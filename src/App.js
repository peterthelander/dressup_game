...
function GirlSVG({ equipped }) {
  const skin = "#FFD9C0";
  const hair = "#F2C761"; // warmer blonde
  return (
    <svg viewBox="0 0 240 360" width={240} height={360}>
      {/* HAIR BACK (bigger so no bald look) */}
      <path d="M48,78 Q120,10 192,78 L192,210 Q120,250 48,210 Z" fill={hair} />
      {/* side locks behind shoulders */}
      <path
        d="M48,120 q-8 28 10 58 q10 16 18 18 v-30 q-10 -16 -12 -46 z"
        fill={hair}
      />
      <path
        d="M192,120 q8 28 -10 58 q-10 16 -18 18 v-30 q10 -16 12 -46 z"
        fill={hair}
      />
      {/* HEAD */}
      <circle cx="120" cy="90" r="42" fill={skin} />
      {/* hairline cap to cover scalp */}
      <path
        d="M78,78 Q120,50 162,78 Q150,60 120,56 Q90,60 78,78 Z"
        fill={hair}
      />
      {/* face */}
      <circle cx="106" cy="86" r="5" fill="#6B7280" />
      <circle cx="134" cy="86" r="5" fill="#6B7280" />
      <path
        d="M102 104 Q120 114 138 104"
        stroke="#6B7280"
        strokeWidth="3"
        fill="none"
        strokeLinecap="round"
      />
      {/* neck */}
      <rect x="112" y="120" width="16" height="14" rx="4" fill={skin} />
      {/* ARMS (under outerwear, above torso) */}
      {/* left arm */}
      <path d="M88 140 q-18 18 -16 46 q2 24 18 28 q10 2 18 -4" fill={skin} />
      <ellipse cx="104" cy="210" rx="10" ry="8" fill={skin} />
      {/* right arm */}
      <path d="M152 140 q18 18 16 46 q-2 24 -18 28 q-10 2 -18 -4" fill={skin} />
      <ellipse cx="136" cy="210" rx="10" ry="8" fill={skin} />
      {/* base torso under clothes */}
      <path d="M90 134 h60 v70 a30 30 0 0 1 -60 0 z" fill="#FFEFE6" />
      {/* DRESS */}
      {equipped.dress && (
        <DressLayer color={equipped.dress.color} hem={equipped.dress.hem} />
      )}
      {/* TOP/BOTTOM when no dress */}
      {!equipped.dress && (
        <>
      ...
      {equipped.accessory && !equipped.accessory.tags.includes("tiara") && (
      ...
      {/* makeup overlay */}
      {equipped.makeup && <MakeupLayer color={equipped.makeup.color} />}
      {/* HAIR FRONT (bangs) */}
      <path
...
function TopLayer({ color }) {
  return <path d="M90 134 h60 v40 h-60 z" fill={color} />;
}
function BottomLayer({ color }) {
  return <path d="M95 174 h50 v26 a22 22 0 0 1 -50 0 z" fill={color} />;
}
function DressLayer({ color, hem = "scallop" }) {
  return hem === "scallop" ? (
    <path d="M85 134 h70 v28 q-35 14 -70 0 z" fill={color} />
  ) : (
    <path d="M85 134 h70 v30 l-70 0 z" fill={color} />
  );
}
function OuterLayer({ color }) {
  return (
    <path d="M80 132 h80 v50 q-40 16 -80 0 z" fill={color} opacity={0.95} />
  );
}
...
function NecklaceLayer({ color }) {
  return (
    <path d="M108 124 q12 10 24 0" stroke={color} strokeWidth="4" fill="none" />
  );
}
function HelmetLayer({ color }) {
  return <path d="M78 64 q42 -28 84 0 v18 h-84 z" fill={color} opacity={0.9} />;
}
function MaskLayer({ color }) {
  return (
    <g opacity={0.9}>
      <rect x="96" y="80" width="48" height="16" rx="8" fill={color} />
      <path d="M96 88 h-10 M144 88 h10" stroke={color} strokeWidth="3" />
    </g>
  );
}
function FinsLayer({ color }) {
  return (
    <g>
      <path d="M88 300 q14 6 28 0 v16 q-14 10 -28 0 z" fill={color} />
      <path d="M124 300 q14 6 28 0 v16 q-14 10 -28 0 z" fill={color} />
    </g>
  );
}
function MakeupLayer({ color }) {
  return (
    <g opacity={0.35}>
      <circle cx="100" cy="96" r="6" fill={color} />
      <circle cx="140" cy="96" r="6" fill={color} />
      <path d="M110 108 q10 8 20 0" stroke={color} strokeWidth="3" />
    </g>
  );
}
